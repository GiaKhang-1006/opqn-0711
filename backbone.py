...
class EdgeFaceBackbone(nn.Module):
    def __init__(self, model_name='edgeface_xxs', feature_dim=512, channel_max=512, size=4, freeze=False):
        super().__init__()
        self.feature_dim = feature_dim
        ...
        # self.backbone.train()
        # === QUAN TRỌNG: ĐÚNG THEO MỤC ĐÍCH ===

        if freeze:
            self.backbone.eval()                    # ← Chỉ inference
            for param in self.backbone.parameters():
                param.requires_grad = False         # ← Freeze
        else:
            self.backbone.train()                   # ← Finetune / Retrain
            # Không cần set requires_grad = True (mặc định)
